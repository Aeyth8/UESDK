...
void UStruct::resolve_field_offsets(uint32_t child_search_start) {
    SPDLOG_INFO("[UStruct] Bruteforcing field offsets...");
    ...
    for (auto i = child_search_start; i < 0x300; i += sizeof(void*)) try {
        ...
        // Uh... todo... check if this is even necessary for the FField variants?
        ...
        for (auto j = sizeof(void*); j < 0x100; ++j) try {
            ...
            const auto possible_name_a1 = possible_fname_a1.to_string_no_numbers();

            if (possible_name_a1 == L"X" || possible_name_a1 == L"Y" || possible_name_a1 == L"Z") {
                if (possible_fname_a1.a1 == possible_fname_a1.a2 && possible_fname_a1.a1 != 0) {
                    SPDLOG_INFO("[FName] Possible case preserving detected, {} == {} for {}", possible_fname_a1.a1, possible_fname_a1.a2, utility::narrow(possible_name_a1));
                    FName::s_is_case_preserving = true;
                }

                FName::s_checked_case_preserving = true;
                ...
                if (!found_ufield_class_offset) {
                    for (auto k = 0; k < 0x100; k += sizeof(void*)) try {
                        ...
                        for (auto l = 0; l < 0x100; l += sizeof(void*)) try {
                            ...
                            const auto possible_name_ffc = possible_fname_ffc.to_string_no_numbers();
                ...
                for (auto k = 0; k < 0x100; k += sizeof(void*)) try {
                    ...
                    const auto potential_next_field_name = potential_next_field_fname.to_string_no_numbers();
                        ...
                        (potential_next_field_name == L"X" || potential_next_field_name == L"Y" || potential_next_field_name == L"Z")) 
                    {
                        ...
                        if (next_next != nullptr) try {
                            ...
                            const auto next_next_name = next_next_fname.to_string_no_numbers();
...
void UStruct::resolve_function_offsets(uint32_t child_search_start) try {
    ...
    for (auto next = gameplay_statics->get_children(); next != nullptr; next = next->get_next()) try {
        ...
        for (auto i = UStruct::s_child_properties_offset + sizeof(void*); i < 0x200; i += sizeof(void*)) try {
            ...
            if (const auto mod_addr = utility::get_module_within(possible_native_fn); mod_addr.has_value()) {
                // Make sure this isn't a vtable.
                const auto possible_addr_within_vtable = *(void**)possible_native_fn;

                if (possible_addr_within_vtable != nullptr && utility::get_module_within(possible_addr_within_vtable).value_or(nullptr) == mod_addr.value()) {
                    continue;
                }
...
void UScriptStruct::update_offsets() {
    ...
    // Look for PropertiesSize
    for (auto i = UStruct::s_super_struct_offset + sizeof(void*); i < 0x300; i += sizeof(int32_t)) try {
        const int32_t properties_size_matrix = *(int32_t*)((uintptr_t)matrix_scriptstruct + i);
        const int32_t properties_size_vector = *(int32_t*)((uintptr_t)vector_scriptstruct + i);
        
        if ((properties_size_matrix == MAT4_SIZE_FLOAT && properties_size_vector == VECTOR_SIZE_FLOAT) ||
            (properties_size_matrix == MAT4_SIZE_DOUBLE && properties_size_vector == VECTOR_SIZE_DOUBLE))
        {
            s_properties_size_offset = i;
            s_min_alignment_offset = i + sizeof(int32_t);

            SPDLOG_INFO("[UScriptStruct] Found PropertiesSize at offset 0x{:X}", s_properties_size_offset);
            SPDLOG_INFO("[UScriptStruct] Found MinAlignment at offset 0x{:X}", s_min_alignment_offset);
            break;
        }
    } catch(...) {
        continue;
