...
void UEngine::exec(UWorld* world, std::wstring_view command, void* output_device) {
    static size_t exec_interface_offset = 0;
    static const auto index = [this]() -> std::optional<uint32_t> {
        SPDLOG_INFO("[UEngine] Searching for UEngine::Exec function...");

        const auto engine = this;

        for (; exec_interface_offset <= 0x100; exec_interface_offset += sizeof(void*)) {
            const auto engine_vtable = *(uintptr_t**)((uintptr_t)engine + exec_interface_offset);

            if (engine_vtable == nullptr || IsBadReadPtr(engine_vtable, 50 * sizeof(void*))) {
                continue;
            }

            for (auto i = 0; i < 50; ++i) {
                const auto fn = engine_vtable[i];

                if ((void*)fn == nullptr || IsBadReadPtr((void*)fn, sizeof(void*))) {
                    continue;
                }

                if (utility::find_string_reference_in_path(fn, L"FLUSHLOG")) {
                    SPDLOG_INFO("[UEngine] Found UEngine::Exec at index {} offset {:x}", i, exec_interface_offset);
                    return i;
                }
            }
        }

        SPDLOG_ERROR("[UEngine] Failed to find UEngine::Exec function!");
        return std::nullopt;
    }();
    ...
    const auto exec_interface = (uintptr_t)this + exec_interface_offset;
    const auto fn = (void(*)(uintptr_t, UWorld*, const wchar_t*, void*))(*((uintptr_t**)exec_interface))[*index];

    fn(exec_interface, world, command.data(), output_device);
}
