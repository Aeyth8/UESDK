...
bool UObject::get_bool_property(std::wstring_view name) const {
    ...
    const auto prop = (sdk::FBoolProperty*)c->find_property(name);

    if (prop == nullptr) {
        return false;
    }

    return prop->get_value_from_object((void*)this);
}

void UObject::set_bool_property(std::wstring_view name, bool value) {
    const auto c = get_class();
    ...
    prop->set_value_in_object((void*)this, value);
}
